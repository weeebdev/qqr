...
class Skew:
    ...
    def shear(self, image, angle, newimage):
        ...
        newimage_x_mid, newimage_y_mid = newimage.shape[0]/2, newimage.shape[1]/2
    ...
    def deskew(self, img):
        ...
        width_AD = np.sqrt(((points[0][0][0] - points[0][1][0]) ** 2) + ((points[0][0][1] - points[0][1][1]) ** 2))
        width_BC = np.sqrt(((points[0][3][0] - points[0][2][0]) ** 2) + ((points[0][3][1] - points[0][2][1]) ** 2))
        maxWidth = max(int(width_AD), int(width_BC))


        height_AB = np.sqrt(((points[0][0][0] - points[0][3][0]) ** 2) + ((points[0][0][1] - points[0][3][1]) ** 2))
        height_CD = np.sqrt(((points[0][2][0] - points[0][1][0]) ** 2) + ((points[0][2][1] - points[0][1][1]) ** 2))
        maxHeight = max(int(height_AB), int(height_CD))

        input_pts = np.float32([points[0][0], points[0][3], points[0][2], points[0][1]])
        ...
        out = np.pad(array=out, pad_width=50, mode='constant', constant_values=255)
        return out