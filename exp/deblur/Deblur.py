...
def non_blind(imblur):
    ...
    imdeblur = cv2.normalize(imdeblur , None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    for i in range(Nz):
        for j in range(Nx):
...
def blind(imblur):
    ...
    finder_clear=io.imread('clear.png', as_gray=True)

    finder_blur = cv2.resize(finder_blur, dsize=(80, 80), interpolation=cv2.INTER_CUBIC)
    finder_clear = cv2.resize(finder_clear, dsize=(80, 80), interpolation=cv2.INTER_CUBIC)
    finder_blur = cv2.normalize(finder_blur , None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    finder_clear = cv2.normalize(finder_clear , None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    show(finder_clear)
    show(finder_blur)
    l=-1000
    for i in range (1, 31, 1):
        for j in range (1, 31, 1):
            ...
            blur1 = cv2.normalize(blur1 , None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
            ...
            blur2 = cv2.normalize(blur2 , None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
            xm=np.mean(blur2)
